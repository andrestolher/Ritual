...
import { Check, Circle, LogOut, Sparkles, Trash2 } from "lucide-react";
...
export default function App() {
  ...
  return <main className="mx-auto min-h-screen max-w-5xl px-5 py-6 sm:px-8 sm:py-10"><header className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-moss">Ritual diario</p><h1 className="mt-1 font-serif text-3xl font-bold">Hola, {user.name.split(" ")[0]}</h1></div><Button variant="ghost" onClick={logout} className="gap-2 text-ink/60"><LogOut size={16} /><span className="hidden sm:inline">Salir</span></Button></header><Card className="relative mt-8 overflow-hidden bg-ink p-6 text-white sm:p-9"><Sparkles className="absolute -right-3 -top-3 text-white/10" size={130} /><p className="text-xs font-bold uppercase tracking-[.18em] text-white/55">Pensamiento de hoy</p><blockquote className="relative mt-3 max-w-2xl font-serif text-2xl leading-snug sm:text-3xl">“{quote?.text}”</blockquote></Card><section className="mt-8 flex items-end justify-between gap-4"><div><h2 className="font-serif text-2xl font-bold">Hoy</h2><p className="mt-1 text-ink/60">{completed}/{habits.length} hábitos completados</p></div><HabitForm habits={habits} onCreated={createHabit} /></section><section className="mt-5 space-y-3">{habits.map((habit) => <HabitRow key={habit.id} habit={habit} onToggle={() => toggle(habit)} onOpen={() => setSelected(habit)} onDelete={() => setDeleting(habit)} />)}{!habits.length && <Card className="border-dashed p-10 text-center"><h3 className="font-serif text-xl font-bold">Tu sistema empieza pequeño</h3><p className="mx-auto mt-2 max-w-sm text-sm text-ink/60">Crea el primer hábito que quieres volver automático.</p></Card>}</section><DeleteConfirm habit={deleting} onConfirm={removeHabit} onCancel={() => setDeleting(null)} /></main>;
}
function HabitRow({ habit, onToggle, onOpen, onDelete }) { const done = habit.todayLog?.completed; return <Card className="group flex items-center gap-3 p-3 sm:p-4"><button onClick={onToggle} aria-label={done ? "Marcar pendiente" : "Marcar completado"} className={`grid h-11 w-11 shrink-0 place-items-center rounded-full border-2 transition ${done ? "border-moss bg-moss text-white" : "border-ink/15 text-transparent hover:border-moss"}`}>{done ? <Check size={20} strokeWidth={3} /> : <Circle size={18} />}</button><button onClick={onOpen} className="min-w-0 flex-1 text-left"><p className="truncate font-semibold">{habit.name}</p><p className="mt-0.5 truncate text-sm text-ink/50">{habit.stackedAfter ? `Después de ${habit.stackedAfter.name}` : habit.identityStatement || (habit.type === "AVOID" ? "Evitar" : "Construir")}</p></button><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${habit.type === "AVOID" ? "bg-amber-100 text-amber-800" : "bg-moss/10 text-moss"}`}>{habit.type === "AVOID" ? "EVITAR" : "CREAR"}</span>{habit.canDelete && <button onClick={() => onDelete(habit)} className="ml-1 rounded-lg p-2 text-ink/25 transition hover:bg-red-50 hover:text-red-500" aria-label="Eliminar hábito"><Trash2 size={16} /></button>}</Card>; }
