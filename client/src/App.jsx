...
import { BarChart3, Check, Circle, LogOut, Sparkles, Trash2 } from "lucide-react";
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      for (let attempt = 0; attempt < 3 && !user; attempt += 1) {
        await load();
        if (!user) await new Promise((resolve) => setTimeout(resolve, 600));
      }
    })();
  }, []);
  ...
  useEffect(() => {
    let active = true;
    function refresh() {
      if (document.visibilityState === "visible" && active) load();
    }
  ...
  async function createHabit(data) {
    const habit = await api("/api/habits", { method: "POST", body: data });
    setHabits((current) => [...current, { ...habit, todayLog: null }]);
  }

  async function toggle(habit) {
    const log = await api(`/api/habits/${habit.id}/log`, { method: "POST", body: { date: today(), completed: !habit.todayLog?.completed } });
    setHabits((current) => current.map((item) => item.id === habit.id ? { ...item, todayLog: log } : item));
  }

  async function saveLog(habit, data) {
    const log = await api(`/api/habits/${habit.id}/log`, { method: "POST", body: { date: today(), ...data } });
    setHabits((current) => current.map((item) => item.id === habit.id ? { ...item, todayLog: log } : item));
  }

  function handleToggle(habit) {
    if (habit.unit || habit.goal != null) setRegistering(habit);
    else toggle(habit);
  }

  async function openHabit(habit) {
    try {
      const list = await refreshHabits();
      window.history.pushState({ view: "habit", id: habit.id }, "", `#habit=${encodeURIComponent(habit.id)}`);
      setSelected(list.find((item) => item.id === habit.id) || null);
    } catch (requestError) {
      setError(requestError.message);
    }
  }
  ...
  function openOverview() {
    window.history.pushState({ view: "overview" }, "", "#overview");
    setSelected(null);
    setShowOverview(true);
  }
  ...
  async function removeHabit() {
    if (!deleting) return;
    await api(`/api/habits/${deleting.id}`, { method: "DELETE" });
    setHabits((current) => current.filter((item) => item.id !== deleting.id));
    setDeleting(null);
  }
